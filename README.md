...
- **Untagged audio tracks are auto-detected.** When ffprobe reports a track without a language tag (Emby just shows "Audio"), Babel Tower runs a Whisper-tiny detection pre-pass on the first 30s of the extracted audio (~2-3s extra per job, model is ~75 MB cached on first use). The detected ISO 639-1 code drives transcription. By default, that detected language is also written back into the source file's audio-stream metadata — `mkvpropedit` for MKV/MKA/WebM (instant, edits the EBML header in place) or `ffmpeg -c copy` remux for MP4/MOV/AVI (no re-encode but I/O bound). Emby's next probe sees the right language. Disable via Settings → Defaults → *Tag detected source language back into the source file* if you want source files left untouched.
